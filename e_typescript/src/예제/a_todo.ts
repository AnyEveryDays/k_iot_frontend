...
function addTodo(todos: TodoItem[], task: string): TodoItem[] {
  const newTodo: TodoItem = {
    // id 값은 기존의 Todo항목 중에서 가장 큰 id값에 1을 더해서 새로운 ID 생성 - 중복 방지

    //? Math.max()
    // : 인자로 주어진 숫자 중에서 가장 큰 수를 반환
    // - 인자 내에 배열이 잇는 경우 배열 요소 중 가장 큰 값을 반환
...
console.log(todos);
todos = addTodo(todos, "HTML 공부");
todos = addTodo(todos, "웹표준 공부");
console.log(todos);

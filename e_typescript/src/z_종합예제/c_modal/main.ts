...
/*
사용자 한 명당 카드 div.user-card 생성.
data-user-id 같은 data-* 속성 대신 dataset.userId 사용(케밥케이스 → camelCase 변환 자동).
innerHTML로 마크업 삽입 — 간결하지만 외부 데이터 삽입 시 XSS 가능성 고려 필요(이 API는 안전하지만, 실무에선 textContent로 개별 삽입 권장).
접근성: 각 카드에 role="button"과 tabindex="0"을 주면 키보드 접근 가능.
dataset.userId 이름: HTML에서는 data-user-id로 렌더되고 JS에서는 dataset.userId로 접근됨.
*/

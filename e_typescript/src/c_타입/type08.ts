...
//? 타입 단언 사용 예시
//: 웹 개발에서 DOM 요소를 조작할 때, 특정 DOM요소를 구체적인 HTMLElement 타입으로 단언!
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // TS 컴파일러는 실시간 코드 변환을 하지 않는다
  // : 코드 수정 시 재컴파일 실행 필요
});

//# 2) JSON 문자열
// - 객체(parse) <> JSON(stringfy)
// : JSON 문자열을 분석(parse)하여 반환되는 데이터는 any타입으로 간주

const jsonString = '{"name": "STK", "age":29 }';

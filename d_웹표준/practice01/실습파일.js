...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 사용자의 추측을 확인하는 함수
  function checkGuess() {
    const resultInfoDiv = document.getElementById('result');
    ...
    if (secretNumber === parseInt(guess)) {
      resultInfoDiv.innerHTML = `<h2>정답입니다</h>
      <p>정답: ${secretNumber}, 총시도횟수: ${attempts}<p>
      `
    }
    // 사용자의 추측이 정답보다 큰 경우
      ...
      <h2>입력값: ${guess} >> 높습니다. 낮춰주세요.</h2>
      `
    }
    else if (secretNumber > guess)
      {
      resultInfoDiv.innerHTML = `
      <h2>입력값: ${guess} >> 낮습니다. 높여주세요.</h2>
      `
    }
    // 사용자의 추측이 정답보다 작은 경우

...
: 검색(필터링)
  >> input 창에 입력되는 내용이 posts 내용에 존재하는 경우
      , 해당 내용만 필터링 되어 출력
...
async function fetchPosts(userId) {
  ...
  return response.json(); //응답을 JS객체로 파싱
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function filterPosts(allPosts, searchTerm){
    return appPosts.filter(post =>  post.title.toLowerCase().includes(searchTerm) || post.body.toLowerCase().includes(searchTerm));

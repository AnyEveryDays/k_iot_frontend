...
// EX) 로그인 상태, 테마 설정(다크./라이트 모드), 다국어 처리 (언어 설정), UI 모달/알림 등

//? 사용 목적
// : props drilling을 없애고 상태 소스의 단일 책임(Single Source of Truch)을 제공
...
// 3) Consumer - Provider로부터 데이터를 받아서 사용하는 컴포넌트
...
const ExampleHeader = () => {
  ...
  if(!user) {
    return <button onClick={() => setUser({name:'손태경'})}>로그인</button>
  }

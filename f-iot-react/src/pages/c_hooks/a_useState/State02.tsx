...
function State02() {
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // 매개변수로 전달받는 e(이벤트 객체)의 target >> 이벤트가 발생된 요소
    // : 이벤트가 발생된 요소
  ...
  return (
    <div>
      ...
      <form onSubmit={handleLoginSubmit} style={{border: '1px solid gray', borderRadius: '10px', display: 'flex', flexDirection: 'column', padding: '20px', justifyContent: 'center', alignItems: 'center', width: '50%', margin: '0 auto'}}>
        <input 
        type="text" 
        name='id'
        value={login.id}
        placeholder='handleLoginChange'
        onChange={handleLoginChange}
        style={{margin: '10px', padding: '5px',width:'50%',alignItems: 'center'}}
      />
      <input 
        type="text" 
        name="password"

...
//   >> 상태값변수명(user) set상태값 변수명(setUser)
...
// const [state, setState] = useState(); 사용 금지
...
function State01() {
  ...
  const [message, setMessage] = useState<string>('안녕하세요'); // 값넣기도 가능하네
  // if (true) {
  ...
  const handleUpClick = () => {
    // setCount(count + 1);  count++ <- 식으로는 재할당 불가
  ...
  return (
    <div>
      <p>카운트 클릭 횟수: {count}</p>
      <button onClick={handleUpClick}>카운트 증가 + 2</button>
      <button onClick={handleDownClick}>카운트 감소 - 2</button>

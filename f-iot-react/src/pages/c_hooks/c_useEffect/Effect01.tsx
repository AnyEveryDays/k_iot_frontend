...
// "컴포넌트가 화면에 나타나거나, 값이 바뀔 때 자동으로 실행되는 코드 저장소"
// 특정 기능을 수행할때의 수행을 시킬 콜백함수다

//? 부수 효과
// : 컴포넌트의 "주요 기능(UI 랜더링, 상태 관리)"외에 발생하는 작업
...
function Effect01() {
  ...
  //# 의존성 배열이 없는 useEffect : 모든 렌더링 시에 동작
  useEffect(()=> {
  ...
  return (
    <div>
      ...
      <br />
      <span>{name}</span>
      <button onClick={() => setName(name === '손태경' ? '손따이경' : '손태경')}> 이름 변경</button>
      <br />
      <p>시간: {time}초</p>
    </div>
  )

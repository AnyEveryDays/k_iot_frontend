...
/*
  ! useEffect 주의사항
  1) 의존성 배열 누락
    : useEffect 내부에서 사용하는 상태나 props값은 반드시 deps에 포함
  2) 무한 렌더링 루프
...
function Practice01() {
  ...
  const fetchReservaions = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP에러! 상태코드: ${response.status}`);
      }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      console.log("예약 데이터 자동 새로고침(30초)");
      fetchReservaions();
  ...
  if (error) return <p>⏹️오류 발생: {error}</p>;

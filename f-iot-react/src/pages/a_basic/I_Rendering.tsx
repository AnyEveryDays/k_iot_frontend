...
// cf) React는 데이터(state, props)가 변경되면 필요한 부분만 다시 랜더링
//    > 화면 전체가 아니라 변경된 컴포넌트만 효율적으로 갱신! (재랜더링)
...
function Item({name, isPacked }: ItemType) {
  ...
  // // 조건 ? 참일 경우 : 거짓일 경우
  // return (
  //   <li>{isPacked ? name +'✔️' : name + '❌' }
  //     </li>
  // )

  //% 3. 논리 연산자를 사용한 조건부 랜더링
  return(
    <li>
      {/* 
        == 논리 연산자 A && B ==
        : 모든 값이 true여야 true 값 반환
        - A가 false면 B값 출력 X
        - A가 true면 B값 출력 O
      */}
      {name} {isPacked && '✔️' }
...
function I_Rendering() {
  ...
  // : 요소 개수 변화 O
  // - 조건에 따른 렌더링
  const businessPeople = peopleDescription.filter(person => person.job === '플라잉맨');
...
// - map()을 통해 여러 요소를 렌더링할 때, key는 React가 어떤 항목이 변겨으 추가, 삭제 되었는지 파악하는 용도로 사용

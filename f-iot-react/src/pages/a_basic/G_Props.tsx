...
/*
  ...
  : Props로 데이터 전달
    <ChildComponent name="손태경" />
    - 함수형 컴포넌트에서 데이터 받는 곳: 매개변수! 
      function ChildComponent({name: string}) { ... }
      % 컴포넌트의 props는 객체로 전달({중괄호에 싸여져 있는 형태})
      % 키="값" 전달은 (객체명 : {키: 데이터타입(<-- 이건 타입스크립트라서 겠지?) }  매개변수 구조로 인식
      %              >> 해당 타입 구조는 타입 별칭으로 정의 가능
      % Props로 전달된 데이터는 자식 컴포넌트에서 readonl처럼 사용
...
function MultiProps( props: {name:string; colorProps: string;}) {
  ...
  // props.name = "SON TAE KYEONG"; - 읽기 전용 속성
...
//=== Props 정리 ===
// 1) 부모 -> 자식: 단방향 데이터 흐름
// 2) 자식은 props를 읽기만 가능하다 readonly
// 3) 상태 변경이 필요하면 부모로부터 콜백 함수를 받아서 수행해야 한다.
...
function G_Props() {
  ...
  return (
    <Wrapper>
      <ChildComponent name='손태경'/>
      <ChildComponent2 name='손인석'/>
      <MultiProps name='손태경' colorProps='blue'/>
      <MultiProps2 name='SON TAEKYEONG' colorProps='lightblue'/>
      <MultiProps2  colorProps='green'
      // 이름 지정하면 지정한값, 지정안하면 위의 '개구리' 대로 나온다. 
      />

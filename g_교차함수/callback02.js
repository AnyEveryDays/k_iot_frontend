...
//? function callback(, index, array) {}  value 값을 삭제한 ,만 넣기  (순서가 중요! 비워지는 인자값의 공간은 남겨둠)
//? function callback(,, array); 이러면 value랑 index가 생략인가
...
// callbackfn(value: string | number | boolean | number[], index: number, array: (string | number | boolean | number[])[]): void
...
const arrowNumbers = numbers.map(value => { 
  return value * value;
});
...
// value, index, array의 순서만 보장되면 콜백 함수의 인장명은 변경 가능
...
cars.forEach(c => console.log(c)); // 보다는 car라는 정식명칭 사용이 권장
...
cars.filter(function(car){
  let isCarLengthBiggerThanFour = car.length > 4;
...
numbersArray.filter(value => value % 2 === 0).map(value => value * value).forEach(value => console.log(value));

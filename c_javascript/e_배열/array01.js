/*
  ...
  == JS 특징 ==
  - 약 타입 언어: 변수 선언 시 타입 필요 없다.
  > 변수 선언 (변수 종류, 변수명/ let age  )

  cf) 자바 변수 선언 (데이터 타입에 변수명 /예) int age)

  - 변수 종류
    > 변수: let - 재선언 불가, 재할당 가능 
    > 변수: var - 재선언 가능, 재할당 가능
...
let fruits =['사과','오렌지','망고']; //대괄호 안에서 ,로 구분후 나열
...
console.log(sports[1]); // 배열명에 인덱스 번호
// $ node array01.js(실행할 파일명.확장자명)

sports[2] = 'basketball'; // 여기서는 재할당 한것 처럼 보여짐
...
sports.length = 6;
// 이어지는 undefined를 < 3empty items> 방법으로 표기
console.log(sports);  // [ '축구', '야구', 'basketball', <3 empty items> ]

sports[5] = '배구';
sports[4] = '탁구';
console.log(sports); // undefined 값이 두개로 줄어들었다
console.log(sports[3]); // [3]으로 콕 집어서 실행하면 undefined 로 나온다.
...
// 2) lastIndexOf(요소값): 배열의 끝 부터 찾는 요소의 첫 번째 인덱스를 반환 (없으면 -1)
...
console.log(hasPotatochip);  // true   존재여부를 참 거짓으로 말한다
...
newLength = fruits.unshift('mango','orange'); // 시작부분에 하나 이상의 요소 추가(,로 나열)
console.log(newLength); // 3
...
//      >> 3번째 인자값이 있으면 추가로 인식!
console.log(fruits);
fruits.splice(1, 0, 'Strawberry','Coconuts'); // 0 : 아무것도 삭제하지 않고 
console.log(fruits); // [ 'mango', 'Strawberry', 'Coconuts', 'orange', 'Banana' ]

// 2)삭제: splice(시작인덱스, 삭제할 요소의 개수)
//      >> 2번째 인자값의 개수만큼 시작인덱스에서 삭제
let removedFruits = fruits.splice(1,2); // 
console.log(removedFruits); // [ 'Strawberry', 'Coconuts' ] 
...
console.log(fruitValues.join()); // 구분자를 전달하지 않으면 기본값 ','  사과,바나나,망고

...
function addTodo() {
  ...
  if (todoInput.value !== ''){
    ...
    // - 보여질 위치의 부모 요소 내에 삽입
    let neweItem = document.createElement('li');

    // 새로운 li태그의 내용값 = input 창의 입력값;
    neweItem.textContent = todoInput.value;

    // 요소에 class 속성 추가
    // : li태그는 동적 이벤트에 따라 동적으로 여러 개 생성 가능
    // : HTML 요소.classList.add('클래스명');
    neweItem.classList.add('todo-item');

    // 생성된 li태그를 ul 태그 내에 삽입
    // : 부모요소.appendChild(자식요소);
    todolist.appendChild(neweItem);
    
    // 태그 추가 할때 이거 사용해보자(addtag)

...
//! 2. Promise 상태 (3가지)
// 1) 대기 (Pending)
// : 초기 상태, 작업이 아직 완료되지 않음(비동기 연산이 완료되지 않음(X))
...
// : new Promise() 생성자 함수를 사용 실무에서는 res , rej 식으로 줄여서 쓴다
// - Promise 내부에 콜백 함수로 비동기 작업이 완료되었을 때의 후속 동작을 정의
//  >> 인자로 성공 시 함수, 실패 시 함수 전달!
//    resolve(value): 프로미스 이행 상태로 변경, 결과값은 value로 반환
//    rejcet(error): 프로미스 실패 상태로 변경, 결과값은 error 반환

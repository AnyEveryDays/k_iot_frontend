...
//! 2. 프로젝트 구현
// ? Book 클래스 : 각 책의 정보 저장 및 대여, 반납 기능 정의
class Book {
  // let id (X)
  ...
  rentBook() {
    ...
    console.log(`${this.title}은(는) 이미 대여 중입니다.`);
    }
  }
  ...
  returnBook() {
    if (!this.isAvailable){
      //isAvailable이 false === 책 반납 가능
      this.isAvailable = true;
      console.log(`${this.title}가 성공적으로 반납되었습니다.`);
    }
  }
}
...
class Library {
  ...
  addBook(title, author) {
    ...
    // this(Library객체).books(객체의 속성-배열).push(객체);
  ...
  displayBook() {
    ...
    this.books.forEach(book =>{
      console.log(`${book.id}: ${book.title} by ${book.author} - ${book.isAvailable ? '대여가능' : '대여중'}`);
    });

  }

  rentBook(id) {
    // +) findIndex: 조건에 일치하는 요소의 index를 반환
    //    find(): 조건에 일치하는 요소 그 자체를 반환   - 배열의 요소 타입
  ...
  updateBook(id, newTitle, newAuthor) {
    ...
    // this.books.find(book => book.title.contain(""))
    //    >> ['신데렐라의 여행', '신데렐라의 추억']
    //    >> '신데렐라의 여행'이 반환
    const book = this.books.find(book => book.id === id);
    ...
    console.log(`책 (id: ${book.id}) 정보가 업데이트되었습니다 : 제목 = ${book.title}, 저자 - ${book.author}`);
  }
  ...
  // 일치하는 저자를 필터링
  // >> 전체 목록 순회 + 각 데이터의 author 값과 매개변수의 author 값이 일치하는 경우 새로운 배열로 반환
  // >> 해당 배열 전체 출력
  //    : filter + forEach

  // cf) 검색 값은 대소문자 구별 X : toLowerCase()로 두 값ㄹ의 형태를 일치시킬 것
  ...
  countAvailableBooks() {
    ...
    // >> isAvailable의 변수를 통해 내부의 boolean 값을 조건으로 사용
...
busanLibrary.addBook('그냥 집가서 자고싶다', '손태경');
busanLibrary.addBook('잠이 보약이다', '손태경');
busanLibrary.addBook('피곤하면 버티지말고 그냥 자라', '손태경');
busanLibrary.addBook('일일 수면 권장 8시간', '손태경');
...
busanLibrary.updateBook(2, '할건 해야제', null);
busanLibrary.updateBook(2, '해야할건 해야지 ㅠㅠ', '손태경');
busanLibrary.displayBook();

// == 양산 도서관 == //
const yangsanLibrary = new Library();

yangsanLibrary.addBook('지금 당장 집가서 자고싶다', '손태경');
yangsanLibrary.addBook('8시간이상의 수면은 보약이다', '손태경');
yangsanLibrary.addBook('식물성 멜라토닌은 신이다', '손태경');
yangsanLibrary.addBook('일일 수면 권장 8시간을 지켜라', '손태경');
...
yangsanLibrary.filterBooksByAuthor('손태경');

...
// - 값(value) : 큐키에 저장할 실제 데이터 (문자열)
// - 만료 날짜(expires) : 쿠키의 유효기간 (없으면 세선 종료 시 삭제)
// - 경로(path): 쿠키가 접근 가능한 경로 범위
// - 도메인(domain): 쿠키가 유효한 도메인 범위
// - Secure: HTTPS 연결에서만 전송 (보안용)
// - HttpOnly
...
// - 쿠키이름, 쿠키값 (필수)
// - 나머지 (선택)
document.cookie = "username=stk; path=/;"
...
// 24시간 === 1일
date.setTime(date.getTime() + (10 * 60 * 1000)); //10분 60초 * 1000밀리초
...
// username=stk; username=stk; userEmail=qwe123
// >> 문자열이기 때문에 직접 파싱
